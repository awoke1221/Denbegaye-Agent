...
export const IconNode = memo(
  ({ data, selected, isDragging, icon, colorTheme, label, isWebhook, ...rest }: IconNodeProps) => {
    ...
    const nodeContainerClasses = clsx(
      'flex flex-col items-center group min-w-[80px] min-h-[80px] relative',
      'node-base node-interactive',
      {
        'node-selected-state node-aura-glow': selected,
        'node-drag-state node-dragging': isDragging,
        'node-hover-state node-float-hover': isHoveringNode && !isDragging,
        'node-click-pulse': isClickAnimating,
      }
    );

    const webhookPulse = isWebhook ? 'animate-pulse-webhook' : '';
      ...
      >
        ...
        {selected && (
          <div
            ...
            style={{
              background: `radial-gradient(circle, ${colorTheme.ring.replace('ring-', 'rgb(').replace(')', ', 0.3)')}1 0%, transparent 70%)`,
              width: '180px',
              height: '180px',
        ...
        {/* Icon container - no circular background */}
        <div
          className={clsx(
        ...
        >
          {/* Large execution ring */}
          {data?.executionState && (
            <div
              className={clsx('absolute inset-[-8px] rounded-full border-4 opacity-90', {
                'border-yellow-300 animate-pulse': data.executionState === 'pending',
                'border-blue-400 border-t-transparent animate-spin':
                  data.executionState === 'executing',
                'border-emerald-500': data.executionState === 'completed',
          ...
          {(() => {
            ...
            return <ProcessedIcon className={clsx('drop-shadow-md', colorTheme.icon)} />;
          })()}
        </div>
        ...
        <span
          className={clsx(
            'mt-2 text-sm font-semibold text-center max-w-[96px] truncate select-none',
            'transition-all duration-300 drop-shadow',
            {
              'text-white drop-shadow-lg scale-105': selected || isHoveringNode,
            }
          )}
          title={label}
          style={{
            textShadow: selected || isHoveringNode ? '0 2px 8px rgba(0, 0, 0, 0.3)' : 'none',
          }}
        >
          {label}
        </span>
