...
function AgentBuilderContent() {
    var _this = this;
    // Context menu state
    var _a = (0, react_1.useState)({ type: null, x: 0, y: 0 }), contextMenu = _a[0], setContextMenu = _a[1];
    var _b = (0, react_1.useState)(false), sidebarCollapsed = _b[0], setSidebarCollapsed = _b[1];
    var _c = (0, react_1.useState)(false), isAdmin = _c[0], setIsAdmin = _c[1];
    var _d = (0, react_1.useState)(false), showTemplateDialog = _d[0], setShowTemplateDialog = _d[1];
    var _e = (0, react_1.useState)({
        ...
        is_public: true,
        version: '1.0.0',
    }), templateForm = _e[0], setTemplateForm = _e[1];
    var _f = (0, react_1.useState)(false), savingTemplate = _f[0], setSavingTemplate = _f[1];
    ...
    // Execution state
    var _g = (0, react_1.useState)({}), executionStatuses = _g[0], setExecutionStatuses = _g[1];
    var _h = (0, react_1.useState)(null), currentExecutionId = _h[0], setCurrentExecutionId = _h[1];
    var socketRef = (0, react_1.useRef)(null);
    var router = (0, navigation_1.useRouter)();
    var _j = (0, AuthContext_1.useAuth)(), user = _j.user, loading = _j.loading;
    var _k = (0, agentBuilderStore_1.useAgentBuilderStore)(), nodes = _k.nodes, edges = _k.edges, selectedNodeId = _k.selectedNodeId, setNodes = _k.setNodes, setEdges = _k.setEdges, selectNode = _k.selectNode, reset = _k.reset;
    ...
    var filteredNodeIds = (0, react_1.useMemo)(function () {
        if (!searchQuery.trim())
            return [];
        ...
        return nodes
            .filter(function (n) {
            var _a, _b, _c, _d, _e;
            return (((_c = (_b = (_a = n.data) === null || _a === void 0 ? void 0 : _a.label) === null || _b === void 0 ? void 0 : _b.toLowerCase) === null || _c === void 0 ? void 0 : _c.call(_b)) || '').includes(q) ||
                n.id.toLowerCase().includes(q) ||
                (((_e = (_d = n.type) === null || _d === void 0 ? void 0 : _d.toLowerCase) === null || _e === void 0 ? void 0 : _e.call(_d)) || '').includes(q);
        })
            .map(function (n) { return n.id; });
    }, [searchQuery, nodes]);
    var filteredEdgeIds = (0, react_1.useMemo)(function () {
        if (!searchQuery.trim())
            return [];
        var q = searchQuery.toLowerCase();
        return edges
    ...
    // Templates state
    var _m = (0, react_1.useState)(''), templateSearchQuery = _m[0], setTemplateSearchQuery = _m[1];
    var _o = (0, react_1.useState)('all'), selectedCategory = _o[0], setSelectedCategory = _o[1];
    var filteredTemplates = (0, react_1.useMemo)(function () {
        return agentBuilderTemplates_1.AgentBuilderTemplates.filter(function (template) {
            var matchesSearch = templateSearchQuery === '' ||
    ...
    var duplicateNode = function (nodeId) {
        ...
        var newNode = __assign(__assign({}, node), { id: "".concat(Date.now()), position: { x: position.x + 40, y: position.y + 40 }, data: __assign(__assign({}, node.data), { label: node.data.label + ' (Copy)' }) });
    ...
    var copyConfig = function (config) {
        ...
        setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ['Config copied to clipboard'], false); });
    };
    ...
    var parseResponseBody = function (response) { return __awaiter(_this, void 0, void 0, function () {
        var text;
    ...
    var applyValidationErrorsToNodes = function (errors) {
        ...
        if (Object.keys(nodesWithErrors).length === 0)
            return;
        var updatedNodes = nodes.map(function (node) {
            var nodeErrors = nodesWithErrors[node.id];
            if (!nodeErrors)
                return node;
            return __assign(__assign({}, node), { data: __assign(__assign({}, node.data), { executionState: 'failed', executionError: nodeErrors.join('; ') }) });
        });
    ...
    var getNodeConfigurationErrors = function (node) {
        ...
        if (type && (type === 'ai' || type.startsWith('ai-'))) {
            if (!config.systemPrompt && !config.messages && !input.prompt && !config.inputText) {
                errors.push("Node ".concat(node.id, ": AI nodes require a prompt or messages configuration"));
            }
        ...
        if (type === 'action-email') {
            if (!config.to && !config.recipients && !input.to) {
                errors.push("Node ".concat(node.id, ": Email action nodes require recipient configuration"));
            }
        }
        ...
        if (type === 'trigger-schedule') {
            if (!config.cronExpression && !config.interval) {
                errors.push("Node ".concat(node.id, ": Schedule trigger nodes require a cron expression or interval"));
            }
        }
        if (type === 'core-code-js' || type === 'core-code-python') {
            if (!config.code && !input.code) {
                errors.push("Node ".concat(node.id, ": Code execution nodes require a code block configuration"));
            }
        }
        if (type === 'logic-delay') {
        ...
        if (type === 'logic-loop') {
            if (!config.iterations && !config.condition) {
                errors.push("Node ".concat(node.id, ": Loop nodes require either iterations count or exit condition"));
            }
        }
        if (type === 'core-set' || type === 'core-transform') {
            if (!config.expression) {
                errors.push("Node ".concat(node.id, ": Transform nodes require an expression configuration"));
            }
        }
        if (type === 'action-save-db') {
            if (!config.table && !config.collection) {
                errors.push("Node ".concat(node.id, ": Database action nodes require table/collection configuration"));
            }
        }
        var metadata = getNodeTypeMetadata(type);
        if ((_c = metadata === null || metadata === void 0 ? void 0 : metadata.configs) === null || _c === void 0 ? void 0 : _c.length) {
    ...
    var validateWorkflowGraph = function (nodes, edges) {
        ...
        var missingTypes = nodes
            .filter(function (node) { return !node.type || !(node.type in nodeTypes_1.nodeTypes); })
            .map(function (node) { return node.type || 'unknown'; });
        if (missingTypes.length > 0) {
            errors.push("Missing or unsupported node types: ".concat(Array.from(new Set(missingTypes)).join(', ')));
        }
        var inDegree = {};
        ...
        var roots = nodes.filter(function (node) { return inDegree[node.id] === 0; });
        if (roots.length === 0) {
    ...
    var _p = (0, react_1.useState)(null), editingNodeId = _p[0], setEditingNodeId = _p[1];
    var _q = (0, react_1.useState)(''), editingNodeLabel = _q[0], setEditingNodeLabel = _q[1];
    ...
    var saveInlineEdit = function () {
        if (editingNodeId) {
            var updatedNodes = nodes.map(function (n) {
                return n.id === editingNodeId ? __assign(__assign({}, n), { data: __assign(__assign({}, n.data), { label: editingNodeLabel }) }) : n;
            });
            setNodes(updatedNodes);
        }
    ...
    var _r = (0, react_1.useState)([]), history = _r[0], setHistory = _r[1];
    var _s = (0, react_1.useState)([]), future = _s[0], setFuture = _s[1];
    var isTimeTravelRef = (0, react_1.useRef)(false);
    var _t = (0, react_1.useState)('Untitled Agent Workflow'), workflowName = _t[0], setWorkflowName = _t[1];
    var _u = (0, react_1.useState)(''), agentDescription = _u[0], setAgentDescription = _u[1];
    var _v = (0, react_1.useState)(null), selectedAgentId = _v[0], setSelectedAgentId = _v[1];
    var _w = (0, react_1.useState)('draft'), currentAgentStatus = _w[0], setCurrentAgentStatus = _w[1];
    var _x = (0, react_1.useState)([]), log = _x[0], setLog = _x[1];
    var _y = (0, react_1.useState)({
        openai: '',
        gemini: '',
        deepseek: '',
        gmail: '',
    }), apiKeys = _y[0], setApiKeys = _y[1];
    var _z = (0, react_1.useState)([]), workflows = _z[0], setWorkflows = _z[1];
    var _0 = (0, react_1.useState)([]), userAgents = _0[0], setUserAgents = _0[1];
    var _1 = (0, react_1.useState)([]), credentials = _1[0], setCredentials = _1[1];
    var _2 = (0, react_1.useState)({
        provider: 'openai',
        label: '',
        apiKey: '',
    }), credentialForm = _2[0], setCredentialForm = _2[1];
    var _3 = (0, react_1.useState)('builder'), activeSection = _3[0], setActiveSection = _3[1];
    var _4 = (0, react_1.useState)(true), showNodePalette = _4[0], setShowNodePalette = _4[1];
    var _5 = (0, react_1.useState)({ x: 80, y: 80 }), nodePalettePosition = _5[0], setNodePalettePosition = _5[1];
    var _6 = (0, react_1.useState)({ x: 80, y: 80 }), settingsPosition = _6[0], setSettingsPosition = _6[1];
    var _7 = (0, react_1.useState)({ x: 80, y: 80 }), nodeConfigPosition = _7[0], setNodeConfigPosition = _7[1];
    var _8 = (0, react_1.useState)(null), draggingPane = _8[0], setDraggingPane = _8[1];
    var _9 = (0, react_1.useState)(false), showLogPanel = _9[0], setShowLogPanel = _9[1];
    var selectedNode = nodes.find(function (n) { return n.id === selectedNodeId; });
    var draftUserAgents = userAgents.filter(function (agent) { return agent.status === 'draft'; });
    var activeUserAgents = userAgents.filter(function (agent) { return agent.status === 'active'; });
    (0, react_1.useEffect)(function () {
        if (typeof window !== 'undefined') {
            setNodeConfigPosition({ x: window.innerWidth - 400, y: 80 });
        }
    }, []);
    var _10 = (0, react_1.useState)({ x: 0, y: 0 }), dragOffset = _10[0], setDragOffset = _10[1];
    var _11 = (0, react_1.useState)(false), isExecuting = _11[0], setIsExecuting = _11[1];
    var _12 = (0, react_1.useState)(null), executionId = _12[0], setExecutionId = _12[1];
    var _13 = (0, react_1.useState)(null), executionStatus = _13[0], setExecutionStatus = _13[1];
    var _14 = (0, react_1.useState)(null), executionResult = _14[0], setExecutionResult = _14[1];
    var _15 = (0, react_1.useState)(null), executionError = _15[0], setExecutionError = _15[1];
    var _16 = (0, react_1.useState)(false), pollingExecution = _16[0], setPollingExecution = _16[1];
    var _17 = (0, react_1.useState)(null), currentExecutingNodeId = _17[0], setCurrentExecutingNodeId = _17[1];
    var backendUrl = process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:3001';
    var _18 = (0, react_1.useState)(false), isLoadingTemplate = _18[0], setIsLoadingTemplate = _18[1];
    ...
    (0, react_1.useEffect)(function () {
        ...
        setHistory(function (h) { return __spreadArray(__spreadArray([], h.slice(-49), true), [{ nodes: nodes, edges: edges }], false); }); // cap 50 entries
        setFuture([]);
    }, [nodes, edges]);
    // Undo handler
    var undoHandler = function () {
        ...
        setFuture(function (f) { return __spreadArray([{ nodes: nodes, edges: edges }], f, true).slice(0, 50); });
    ...
    var redoHandler = function () {
        ...
        var next = future[0];
        isTimeTravelRef.current = true;
        setHistory(function (h) { return __spreadArray(__spreadArray([], h.slice(-49), true), [next], false); });
        setHistory(function (h) { return __spreadArray(__spreadArray([], h.slice(-49), true), [next], false); });
        setNodes(next.nodes);
        setEdges(next.edges);
        setFuture(function (f) { return f.slice(1); });
    };
    // Keyboard shortcuts
    (0, react_1.useEffect)(function () {
    ...
    (0, react_1.useEffect)(function () {
        ...
        var initializeExecutionState = function (executionId) {
            ...
            setExecutionStatuses(function (prev) {
                var _a;
                return (__assign(__assign({}, prev), (_a = {}, _a[executionId] = {
                    status: 'running',
                    nodeStatuses: {},
                }, _a)));
            });
        ...
        socket.on('node-completed', function (data) {
            ...
            setNodes(nodesRef.current.map(function (node) {
                return node.id === data.nodeId
                    ? __assign(__assign({}, node), { data: __assign(__assign({}, node.data), { executionState: data.success ? 'completed' : 'failed', executionError: data.success
                                ? null
                                : data.error || data.errorStack || 'Unknown error' }) }) : node;
            }));
            var node = nodesRef.current.find(function (n) { return n.id === data.nodeId; });
            if (data.success) {
    ...
    var createCredential = function () { return __awaiter(_this, void 0, void 0, function () {
        var token, response, data;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    if (!(currentUser === null || currentUser === void 0 ? void 0 : currentUser.id)) {
                        setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ['Please sign in to save credentials'], false); });
                        return [2 /*return*/];
                    }
                    return [4 /*yield*/, getAuthToken()];
                case 1:
                    token = _a.sent();
                    if (!token) {
                        setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ['Unable to retrieve auth token'], false); });
                        return [2 /*return*/];
                    }
                    return [4 /*yield*/, fetch('/api/credentials', {
                            method: 'POST',
                ...
                case 3:
                    data = _a.sent();
                    if (data.error) {
                        setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ["Credential save failed: ".concat(data.error)], false); });
                        return [2 /*return*/];
                    }
                    setCredentialForm({ provider: 'openai', label: '', apiKey: '' });
                    loadCredentials();
                    setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ["Credential saved for ".concat(credentialForm.provider)], false); });
                    return [2 /*return*/];
            }
        });
    }); };
    var saveApiKeys = function () {
        ...
        setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ['API keys saved'], false); });
    };
    var exportAgentJson = function (agent) {
        ...
        setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ["Exported agent JSON: ".concat(agent.name || 'Untitled Agent')], false); });
    };
    var normalizeAgentStatus = function (status) { return (status === 'active' ? 'active' : 'draft'); };
    var loadCredentialIntoNode = function (provider, nodeId) {
        var targetNode = nodes.find(function (n) { return n.id === nodeId || selectedNodeId; });
        if (!targetNode)
            return;
        var updatedNodes = nodes.map(function (n) {
            return n.id === targetNode.id
                ? __assign(__assign({}, n), { data: __assign(__assign({}, n.data), { config: __assign(__assign({}, (n.data.config || {})), { credentialProvider: provider }) }) }) : n;
        });
        setNodes(updatedNodes);
    };
    var loadUserAgent = function (agent) {
        ...
        setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ["Loaded agent \"".concat(agent.name, "\" into the builder.")], false); });
    };
    var exportDashboardReport = function () {
        ...
        anchor.remove();
        URL.revokeObjectURL(url);
        setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ['Exported dashboard report successfully'], false); });
    };
    var clearExecutions = function () {
        setExecutionStatuses({});
        setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ['Cleared execution history'], false); });
    };
    var createNewAgentWorkflow = function () {
        ...
        setEdges([]);
        setActiveSection('builder');
        setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ['Started a new agent workflow'], false); });
        localStorage.removeItem('agent-builder-workflow');
    };
    var publishAgent = function () { return __awaiter(_this, void 0, void 0, function () {
        var error;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    if (!selectedAgentId) {
                        setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ['Save draft first before publishing'], false); });
                        return [2 /*return*/];
                    }
                    return [4 /*yield*/, supabaseClient_1.supabase
                            .from('user_agents')
                ...
                case 1:
                    error = (_a.sent()).error;
                    if (error) {
                        setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ["Failed to publish agent: ".concat(error.message)], false); });
                        return [2 /*return*/];
                    }
                    setCurrentAgentStatus('active');
                    setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ["Agent published: ".concat(workflowName)], false); });
    ...
    var deleteAgent = function (agentId) { return __awaiter(_this, void 0, void 0, function () {
        var confirmed, error;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    if (!confirmed)
                        return [2 /*return*/];
                    return [4 /*yield*/, supabaseClient_1.supabase.from('user_agents').delete().eq('id', agentId)];
                case 1:
                    error = (_a.sent()).error;
                    if (error) {
                        setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ["Delete failed: ".concat(error.message)], false); });
                        return [2 /*return*/];
                    }
                ...
                case 2:
                    _a.sent();
                    setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ['Agent deleted successfully'], false); });
                    return [2 /*return*/];
            }
        });
    }); };
    var setAgentStatus = function (agentId, status) { return __awaiter(_this, void 0, void 0, function () {
        var error;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, supabaseClient_1.supabase.from('user_agents').update({ status: status }).eq('id', agentId)];
                case 1:
                    error = (_a.sent()).error;
                    if (error) {
                        setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ["Failed to set status: ".concat(error.message)], false); });
                        return [2 /*return*/];
                    }
                    return [4 /*yield*/, loadUserAgents()];
                case 2:
    ...
    var nodeTypeCategory = function (nodeType) {
        var info = nodeTypes_1.availableNodeTypes.find(function (item) { return item.id === nodeType; });
    ...
    var getNodeTypeMetadata = function (type) {
        return type ? nodeTypes_1.availableNodeTypes.find(function (item) { return item.id === type; }) : undefined;
    };
    var isNodeConfigured = function (node) {
        var _a, _b;
        var config = ((_a = node.data) === null || _a === void 0 ? void 0 : _a.config) || {};
        var metadata = getNodeTypeMetadata(node.type);
        if ((_b = metadata === null || metadata === void 0 ? void 0 : metadata.configs) === null || _b === void 0 ? void 0 : _b.length) {
    ...
    var getGeminiActionLevel = function (action) {
        ...
        if (beginnerActions.includes(action || ''))
            return 'Beginner';
        if (intermediateActions.includes(action || ''))
            return 'Intermediate';
    ...
    var updateNodeConfig = function (nodeId, configUpdates) {
        var updatedNodes = nodes.map(function (n) {
            ...
            if (n.type === 'data-google-sheets' && configUpdates.action) {
                updatedLabel = "Google Sheets \u2014 ".concat(formatGoogleSheetsActionLabel(configUpdates.action));
            }
            else if (n.type === 'ai-gemini' && configUpdates.action) {
                updatedLabel = "Gemini \u2014 ".concat(formatGeminiActionLabel(configUpdates.action));
            }
            var updatedData = __assign(__assign({}, n.data), { label: updatedLabel, config: __assign(__assign({}, newConfig), { isConfigured: isNodeConfigured(__assign(__assign({}, n), { data: __assign(__assign({}, n.data), { config: newConfig }) })) }) });
    ...
    (0, react_1.useEffect)(function () {
        if (typeof window === 'undefined' || oauthRedirectHandledRef.current)
            return;
        ...
        if (!oauthService)
            return;
        var restoreServiceToken = function () { return __awaiter(_this, void 0, void 0, function () {
                ...
                switch (_c.label) {
                    ...
                    case 1:
                        ...
                        if (token) {
                            ...
                            window.localStorage.setItem("serviceToken:".concat(oauthService), JSON.stringify(storedToken));
                            if (oauthNodeId) {
                                targetNode = nodes.find(function (n) { return n.id === oauthNodeId; });
                                if (targetNode) {
                                    serviceTokens = __assign(__assign({}, (((_b = targetNode.data.config) === null || _b === void 0 ? void 0 : _b.serviceTokens) || {})), (_a = {}, _a[oauthService] = storedToken, _a));
                                    updateNodeConfig(oauthNodeId, { serviceTokens: serviceTokens });
                                }
                            }
                            setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ["Connected ".concat(oauthService, " account successfully.")], false); });
                        }
                        return [3 /*break*/, 4];
                    case 2:
                        error_1 = _c.sent();
                    ...
                    case 3:
                        ...
                        cleanUrl = "".concat(window.location.pathname).concat(params.toString() ? "?".concat(params.toString()) : '').concat(window.location.hash);
        ...
        var stepThroughExecution = function () {
            ...
            if (node) {
                ...
                setNodes(nodes.map(function (n) {
                    return n.id === nodeId
                        ? __assign(__assign({}, n), { data: __assign(__assign({}, n.data), { executionState: 'executing' }) }) : n;
                }));
                setLog(function (prev) { var _a; return __spreadArray(__spreadArray([], prev, true), ["Executing node: ".concat(((_a = node.data) === null || _a === void 0 ? void 0 : _a.label) || nodeId)], false); });
                ...
                setTimeout(function () {
                    ...
                    setNodes(nodes.map(function (n) {
                        return n.id === nodeId
                            ? __assign(__assign({}, n), { data: __assign(__assign({}, n.data), { executionState: 'completed' }) }) : n;
                    }));
                    setLog(function (prev) { var _a; return __spreadArray(__spreadArray([], prev, true), ["Node completed: ".concat(((_a = node.data) === null || _a === void 0 ? void 0 : _a.label) || nodeId)], false); });
                    currentIndex++;
    ...
    var fetchExecutionStatus = function (executionIdToPoll) { return __awaiter(_this, void 0, void 0, function () {
        ...
        return __generator(this, function (_c) {
            switch (_c.label) {
                ...
                case 1:
                    ...
                    accessToken = (_b = sessionData === null || sessionData === void 0 ? void 0 : sessionData.session) === null || _b === void 0 ? void 0 : _b.access_token;
                    if (!accessToken) {
                ...
                case 2:
                    ...
                    setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ["Execution ".concat(executionIdToPoll, " status: ").concat(execution_1.status)], false); });
                    if (execution_1.status === 'completed' || execution_1.status === 'failed') {
                        setPollingExecution(false);
                        setCurrentExecutingNodeId(null);
                        // Reset all node execution states
                        setNodes(nodes.map(function (node) { return (__assign(__assign({}, node), { data: __assign(__assign({}, node.data), { executionState: null }) })); }));
                    }
                    return [3 /*break*/, 4];
                case 3:
                    error_2 = _c.sent();
                    setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), [
                        "Execution poll error: ".concat(error_2 instanceof Error ? error_2.message : error_2),
                    ], false); });
                    return [3 /*break*/, 4];
                case 4: return [2 /*return*/];
            }
    ...
    var sanitizeWorkflowForExecution = function (nodes, edges) {
        var sanitizedNodes = nodes.map(function (node) {
            ...
            return ({
                ...
                data: node.data,
                config: (_b = (_a = node.data) === null || _a === void 0 ? void 0 : _a.config) !== null && _b !== void 0 ? _b : {},
            });
        });
        var sanitizedEdges = edges.map(function (edge) { return ({
            id: edge.id,
            source: edge.source,
            target: edge.target,
    ...
    var executeWorkflow = function () { return __awaiter(_this, void 0, void 0, function () {
        var execOrder, nodesWithResetState, validationErrors, message_1, _a, sanitizedNodes, sanitizedEdges, sessionData, accessToken, proposedExecutionId, socket, payload, response, responseBody, validationErrors_1, message_2, serverError, message, data, actualExecutionId_1, error_3, message_3;
        var _b, _c;
        return __generator(this, function (_d) {
            switch (_d.label) {
                case 0:
                    ...
                    setCurrentExecutingNodeId(null);
                    setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ['Starting workflow execution...'], false); });
                    _d.label = 1;
                case 1:
                    _d.trys.push([1, 5, 6, 7]);
                    execOrder = computeExecOrder();
                    nodesWithResetState = nodes.map(function (node) { return (__assign(__assign({}, node), { data: __assign(__assign({}, node.data), { executionState: null }) })); });
                    ...
                    if (validationErrors.length > 0) {
                        ...
                        setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), [message_1], false); });
                        return [2 /*return*/];
                    }
                    _a = sanitizeWorkflowForExecution(nodes, edges), sanitizedNodes = _a.sanitizedNodes, sanitizedEdges = _a.sanitizedEdges;
                    return [4 /*yield*/, supabaseClient_1.supabase.auth.getSession()];
                case 2:
                    ...
                    return [4 /*yield*/, fetch("".concat(backendUrl, "/api/agent-run"), {
                            method: 'POST',
                            headers: {
                ...
                case 4:
                    ...
                    if (!response.ok) {
                        ...
                        if (responseBody &&
                            ...
                            Array.isArray((_c = responseBody.details) === null || _c === void 0 ? void 0 : _c.errors)) {
                            validationErrors_1 = responseBody.details.errors.filter(function (item) { return typeof item === 'string'; });
                            if (validationErrors_1.length > 0) {
                                ...
                                setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), [message_2], false); });
                                applyValidationErrorsToNodes(validationErrors_1);
                                return [2 /*return*/];
                            }
                        }
                        serverError = responseBody && typeof responseBody === 'object'
                            ? responseBody.error ||
                                responseBody.message ||
                                JSON.stringify(responseBody)
                            : String(responseBody || "".concat(response.statusText));
                        message = "Agent execution request failed (".concat(response.status, " ").concat(response.statusText, ") - ").concat(serverError);
                    ...
                    if (!actualExecutionId_1) {
                        socket.emit('unsubscribe:execution', proposedExecutionId);
                        throw new Error("Agent execution succeeded but no executionId returned. Response body: ".concat(JSON.stringify(data)));
                    }
                    ...
                    setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ["Agent execution queued successfully: ".concat(actualExecutionId_1)], false); });
                    if (data.reused) {
                        setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ['Existing execution reused due to idempotency.'], false); });
                    }
                    return [3 /*break*/, 7];
                case 5:
                    error_3 = _d.sent();
                    ...
                    setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ["Execution error: ".concat(message_3)], false); });
                    return [3 /*break*/, 7];
                case 6:
                    setIsExecuting(false);
                    return [7 /*endfinally*/];
    ...
    (0, react_1.useEffect)(function () {
        if (!loading && user) {
            var checkAdmin = function () { return __awaiter(_this, void 0, void 0, function () {
                var _a, profile, error, error_4;
                return __generator(this, function (_b) {
                    switch (_b.label) {
                        case 0:
                            _b.trys.push([0, 2, , 3]);
                            console.log('Checking admin status for user:', user === null || user === void 0 ? void 0 : user.id);
                            return [4 /*yield*/, supabaseClient_1.supabase
                                    .from('profiles')
                                    .select('role')
                                    .eq('id', user.id)
                        ...
                        case 1:
                            ...
                            console.log('Admin check result:', { profile: profile, error: error, role: profile === null || profile === void 0 ? void 0 : profile.role });
                            if (!error && (profile === null || profile === void 0 ? void 0 : profile.role) === 'admin') {
        ...
        if (templateData) {
            ...
            setTimeout(function () {
                try {
                    ...
                    setSelectedAgentId(null);
                    setCurrentAgentStatus('draft');
                    setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ["Template \"".concat(template_1.name, "\" loaded successfully")], false); });
                    // Clear any existing saved workflow
                    localStorage.removeItem('agent-builder-workflow');
                }
                catch (error) {
                    console.error('Error loading template:', error);
                    setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ['Error loading template: Invalid template data'], false); });
                }
            ...
            setTimeout(function () {
                try {
                    ...
                    setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ['Failed to create new workflow'], false); });
                }
                setIsLoadingTemplate(false);
            }, 100);
        }
            ...
            if (stored) {
                ...
                var defaultTemplate_1 = agentBuilderTemplates_1.AgentBuilderTemplates.find(function (t) { return t.id === 'social-content-generator'; });
                if (defaultTemplate_1) {
                    ...
                    setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ["Default template \"".concat(defaultTemplate_1.name, "\" loaded")], false); });
                }
            }
        }
    ...
    var onEdgeContextMenu = function (event, edge) {
        ...
        if (window.confirm('Remove this connection?')) {
            setEdges(edges.filter(function (e) { return e.id !== edge.id; }));
        }
    };
    // Advanced: highlight compatible ports on connect
    var _20 = (0, react_1.useState)(null), connectingNodeId = _20[0], setConnectingNodeId = _20[1];
    ...
    var _21 = (0, react_1.useState)({}), collapsedGroups = _21[0], setCollapsedGroups = _21[1];
    var addNode = function (type) {
        var nodeType = nodeTypes_1.availableNodeTypes.find(function (t) { return t.id === type; });
        if (!nodeType)
            return;
            ...
            var newGroup = {
                ...
                data: {
                    ...
                    onToggleCollapse: function (id) {
                        ...
                        var updatedNodes = nodes.map(function (n) {
                            return n.id === id ? __assign(__assign({}, n), { data: __assign(__assign({}, n.data), { collapsed: !collapsedGroups[id] }) }) : n;
                        });
                        setNodes(updatedNodes);
                    },
                },
    ...
    var loadTemplate = function (templateId) {
        var template = agentBuilderTemplates_1.AgentBuilderTemplates.find(function (t) { return t.id === templateId; });
        if (template) {
            ...
            setAgentDescription('');
            setCurrentAgentStatus('draft');
            setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ["Template \"".concat(template.name, "\" loaded successfully")], false); });
        }
        else {
            setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ["Template \"".concat(templateId, "\" not found")], false); });
        }
    };
    var renderCredentialSelect = function (node, title) {
        ...
        var saved = credentials.filter(function (cred) {
            return cred.provider === nodeType.replace('ai-', '') ||
                cred.provider === nodeType.replace('comm-', '').replace('-oauth2', '').replace('-bot', '');
        });
        return (<div>
        <label_1.Label className="text-xs font-medium text-slate-600 dark:text-slate-300">{title}</label_1.Label>
        <select_1.Select value={((_a = node.data.config) === null || _a === void 0 ? void 0 : _a.credentialProvider) || ''} onValueChange={function (value) { return updateNodeConfig(node.id, { credentialProvider: value }); }}>
          <select_1.SelectTrigger className="mt-1 w-full">
            <select_1.SelectValue placeholder="Choose saved credential"/>
          </select_1.SelectTrigger>
          <select_1.SelectContent>
            {saved.map(function (cred) { return (<select_1.SelectItem key={cred.id} value={cred.provider}>
    ...
    var saveAgentAsDraft = function () { return __awaiter(_this, void 0, void 0, function () {
        var _a, profile, profileError, createError_1, agentStatus, agentPayload, error_5, _b, data, error_6;
        var _c, _d;
        return __generator(this, function (_e) {
            switch (_e.label) {
                case 0:
                    if (!(currentUser === null || currentUser === void 0 ? void 0 : currentUser.id)) {
                        setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ['Cannot save: not signed in'], false); });
                        return [2 /*return*/];
                    }
                    return [4 /*yield*/, supabaseClient_1.supabase
                            .from('profiles')
                ...
                case 1:
                    ...
                    if (profileError && profileError.code !== 'PGRST116') {
                        ...
                        setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ["Failed to check profile: ".concat(profileError.message)], false); });
                        return [2 /*return*/];
                    }
                    if (!!profile) return [3 /*break*/, 3];
                    return [4 /*yield*/, supabaseClient_1.supabase.from('profiles').insert({
                            id: currentUser.id,
                            email: currentUser.email,
                            full_name: ((_c = currentUser.user_metadata) === null || _c === void 0 ? void 0 : _c.full_name) || '',
                            avatar_url: ((_d = currentUser.user_metadata) === null || _d === void 0 ? void 0 : _d.avatar_url) || '',
                        })];
                case 2:
                    createError_1 = (_e.sent()).error;
                    if (createError_1) {
                        setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ["Failed to create profile: ".concat(createError_1.message)], false); });
                        return [2 /*return*/];
                    }
                    _e.label = 3;
                case 3:
                ...
                case 4:
                    ...
                    if (error_5) {
                        setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ["Failed to update agent: ".concat(error_5.message)], false); });
                        return [2 /*return*/];
                    }
                    localStorage.setItem('agent-builder-workflow', JSON.stringify({
                        nodes: nodes,
                    ...
                    setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ["Agent updated: ".concat(workflowName)], false); });
                    return [3 /*break*/, 7];
                ...
                case 6:
                    ...
                    if (error_6 || !data) {
                        setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ["Failed to save draft: ".concat((error_6 === null || error_6 === void 0 ? void 0 : error_6.message) || 'unknown')], false); });
                        return [2 /*return*/];
                    }
                    ...
                    setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ["Draft saved: ".concat(workflowName)], false); });
                    _e.label = 7;
                case 7:
    ...
    var openSaveAsTemplateDialog = function () {
        if (!isAdmin) {
            setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ['Save as template is available only to admin users.'], false); });
            return;
        }
        setTemplateForm(function (prev) { return (__assign(__assign({}, prev), { name: workflowName || prev.name || 'Untitled Agent Workflow', description: agentDescription || prev.description })); });
        setShowTemplateDialog(true);
    };
    var saveCurrentWorkflowAsTemplate = function () { return __awaiter(_this, void 0, void 0, function () {
        ...
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    if (!isAdmin || !user) {
                        setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ['Unable to save template: admin access required.'], false); });
                        return [2 /*return*/];
                    }
                    if (!(workflowName === null || workflowName === void 0 ? void 0 : workflowName.trim())) {
                        setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ['Please give your workflow a name before saving as a template.'], false); });
                        return [2 /*return*/];
                    }
                ...
                case 2:
                    ...
                    return [4 /*yield*/, fetch('/api/admin/templates', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                Authorization: "Bearer ".concat(token),
                ...
                case 3:
                    ...
                    return [4 /*yield*/, response_1
                            .json()
                            .catch(function () { return ({ error: 'Unknown error', status: response_1.status }); })];
                case 4:
                    errorData = _b.sent();
                    console.error('Template save failed:', errorData.error || errorData.message || "HTTP ".concat(response_1.status, ": ").concat(response_1.statusText));
                    ...
                    throw new Error(errorData.error || errorData.message || "HTTP ".concat(response_1.status, ": ").concat(response_1.statusText));
                case 5:
                    setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ["Template saved successfully: ".concat(templatePayload_1.name)], false); });
                    setShowTemplateDialog(false);
                    return [3 /*break*/, 8];
                case 6:
                    error_7 = _b.sent();
                    message_4 = error_7 instanceof Error ? error_7.message : 'Unknown error';
                    setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ["Failed to save template: ".concat(message_4)], false); });
    ...
    var executeWorkflowHandler = function () { return __awaiter(_this, void 0, void 0, function () {
        return __generator(this, function (_a) {
            // TODO: implement workflow execution
            setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ['Workflow execution started'], false); });
            return [2 /*return*/];
        });
    }); };
    var createNewAgentWorkflowHandler = function () {
        ...
        reset();
        setWorkflowName('Untitled Agent Workflow');
        setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ['New workflow created'], false); });
    };
    var publishAgentHandler = function () { return __awaiter(_this, void 0, void 0, function () {
        return __generator(this, function (_a) {
            // TODO: implement publishing agent
            setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ['Agent published'], false); });
            return [2 /*return*/];
        });
    }); };
    var resetHandler = function () {
        ...
        setNodes([]);
        setEdges([]);
        setLog(function (prev) { return __spreadArray(__spreadArray([], prev, true), ['Workflow reset'], false); });
    };
    return (<div className="h-screen flex bg-gradient-to-br from-slate-100 via-cyan-100 to-indigo-100 dark:from-slate-950 dark:via-slate-900 dark:to-indigo-950">
      <AgentBuilderSidebar_1.AgentBuilderSidebar sidebarCollapsed={sidebarCollapsed} setSidebarCollapsed={setSidebarCollapsed} activeSection={activeSection} setActiveSection={setActiveSection} user={user}/>

      {/* Main Content */}
      <div className="flex-1 flex flex-col">
        {activeSection === 'builder' && (<>
            <ExecutionControls_1.ExecutionControls workflowName={workflowName} setWorkflowName={setWorkflowName} currentAgentStatus={currentAgentStatus} executionStatus={executionStatus} currentExecutionId={currentExecutionId} executionError={executionError} showNodePalette={showNodePalette} setShowNodePalette={setShowNodePalette} saveAgentAsDraft={saveAgentAsDraft} executeWorkflow={executeWorkflow} createNewAgentWorkflow={createNewAgentWorkflow} publishAgent={publishAgent} openSaveAsTemplateDialog={openSaveAsTemplateDialog} isAdmin={isAdmin} reset={reset} isExecuting={isExecuting}/>
              ...
              <dialog_1.DialogContent className="max-w-2xl">
                ...
                <div className="space-y-4 py-2">
                  <div>
                    ...
                    <input_1.Input id="template-name" value={templateForm.name} onChange={function (e) { return setTemplateForm(function (prev) { return (__assign(__assign({}, prev), { name: e.target.value })); }); }} placeholder="Agentic workflow template name" className="mt-1"/>
                  </div>
                  <div>
                    <label_1.Label htmlFor="template-description">Description</label_1.Label>
                    <textarea_1.Textarea id="template-description" value={templateForm.description} onChange={function (e) {
                return setTemplateForm(function (prev) { return (__assign(__assign({}, prev), { description: e.target.value })); });
            }} placeholder="Brief description of the workflow template" rows={3}/>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label_1.Label htmlFor="template-category">Category</label_1.Label>
                      <select_1.Select value={templateForm.category} onValueChange={function (value) {
                return setTemplateForm(function (prev) { return (__assign(__assign({}, prev), { category: value })); });
                        ...
                        <select_1.SelectContent>
                          {templateCategories.map(function (category) { return (<select_1.SelectItem key={category} value={category}>
                              {category}
                            </select_1.SelectItem>); })}
                        </select_1.SelectContent>
                      </select_1.Select>
                    ...
                    <div className="flex items-end gap-2">
                      <checkbox_1.Checkbox id="template-public" checked={templateForm.is_public} onCheckedChange={function (checked) {
                return setTemplateForm(function (prev) { return (__assign(__assign({}, prev), { is_public: checked })); });
            }}/>
                ...
                <dialog_1.DialogFooter>
                  <button_1.Button variant="outline" onClick={function () { return setShowTemplateDialog(false); }}>
                    Cancel
                  </button_1.Button>
                  <button_1.Button onClick={saveCurrentWorkflowAsTemplate} disabled={savingTemplate}>
            ...
            <AgentBuilderCanvas_1.AgentBuilderCanvas activeSection={activeSection} nodes={nodes} edges={edges} onNodesChange={onNodesChange} onEdgesChange={onEdgesChange} onConnect={onConnect} onConnectStart={onConnectStart} onConnectEnd={onConnectEnd} onNodeClick={onNodeClick} onNodeDoubleClick={onNodeDoubleClick} onPaneClick={onPaneClick} onPaneContextMenu={onPaneContextMenu} onNodeContextMenu={onNodeContextMenu} onEdgeContextMenu={onEdgeContextMenu} filteredEdgeIds={filteredEdgeIds} workflows={workflows} setNodes={setNodes} setEdges={setEdges} setWorkflowName={setWorkflowName} setActiveSection={setActiveSection} setLog={setLog}/>

            <NodeManagement_1.NodeManagement activeSection={activeSection} showNodePalette={showNodePalette} setShowNodePalette={setShowNodePalette} searchQuery={searchQuery} setSearchQuery={setSearchQuery} nodePalettePosition={nodePalettePosition} settingsPosition={settingsPosition} nodeConfigPosition={nodeConfigPosition} draggingPane={draggingPane} setDraggingPane={setDraggingPane} dragOffset={dragOffset} setDragOffset={setDragOffset} selectedNode={selectedNode} selectedNodeId={selectedNodeId !== null && selectedNodeId !== void 0 ? selectedNodeId : null} nodes={nodes} edges={edges} credentials={credentials} credentialForm={credentialForm} setCredentialForm={setCredentialForm} loadCredentials={loadCredentials} loadCredentialIntoNode={loadCredentialIntoNode} updateNodeConfig={updateNodeConfig} apiKeys={apiKeys} setApiKeys={setApiKeys} saveApiKeys={saveApiKeys} renderIcon={renderIcon} selectNode={selectNode} copyConfig={copyConfig} duplicateNode={duplicateNode} duplicateEdge={duplicateEdge} undo={undoHandler} redo={redoHandler} setContextMenu={setContextMenu} contextMenu={contextMenu} setNodes={setNodes} setEdges={setEdges} setActiveSection={setActiveSection} setLog={setLog} nodeTypeCategory={nodeTypeCategory} isNodeConfigured={isNodeConfigured}/>

            <AgentBuilderLogPanel_1.AgentBuilderLogPanel showLogPanel={showLogPanel} setShowLogPanel={setShowLogPanel} log={log}/>
          </>)}

        {activeSection === 'dashboard' && (<AgentBuilderDashboard_1.AgentBuilderDashboard user={user} workflows={workflows} userAgents={userAgents} executionStatuses={executionStatuses} draftUserAgents={draftUserAgents} activeUserAgents={activeUserAgents} loadUserAgent={loadUserAgent} createNewAgentWorkflow={createNewAgentWorkflow} exportDashboardReport={exportDashboardReport} clearExecutions={clearExecutions} setActiveSection={setActiveSection} exportAgentJson={exportAgentJson} deleteAgent={deleteAgent}/>)}

        {activeSection === 'templates' && (<AgentBuilderTemplates_1.AgentBuilderTemplates filteredTemplates={filteredTemplates} templateSearchQuery={templateSearchQuery} setTemplateSearchQuery={setTemplateSearchQuery} selectedCategory={selectedCategory} setSelectedCategory={setSelectedCategory} loadTemplate={loadTemplate} setActiveSection={setActiveSection} renderIcon={renderIcon} setLog={setLog}/>)}

        {activeSection === 'webhooks' && (<div className="flex-1 p-6">
            <webhook_manager_1.WebhookManager workflows={workflows}/>
          </div>)}

        {activeSection === 'vault' && (<AgentBuilderVault_1.AgentBuilderVault credentials={credentials} credentialForm={credentialForm} setCredentialForm={setCredentialForm} createCredential={createCredential}/>)}

        {activeSection === 'settings' && (<AgentBuilderSettings_1.AgentBuilderSettings apiKeys={apiKeys} setApiKeys={setApiKeys} saveApiKeys={saveApiKeys} showNodePalette={showNodePalette} setShowNodePalette={setShowNodePalette}/>)}
      </div>
    </div>);
}
