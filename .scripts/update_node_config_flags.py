...
required_labels = {
    ...
    'Phone Number', 'Spreadsheet ID', 'Method', 'URL', 'Code', 'Calendar ID'
}
optional_labels = {
...
for line in lines:
    ...
    if inside_configs:
        if inside_obj:
            ...
            if stripped.startswith("l:") and "'" in line:
                parts = line.split("l:", 1)[1].strip()
                if parts.startswith("'"):
                    current_label = parts.split("'", 2)[1]
            if brace_level == 0:
                ...
                if current_label in required_labels:
                    if 'required:' not in obj_text:
                        insert = True
                        insert_value = True
                elif current_label in optional_labels:
                    if 'required:' not in obj_text:
                        insert = True
                        insert_value = False
                if insert:
                    for i in range(len(current_obj_lines) - 1, -1, -1):
                        if current_obj_lines[i].strip().startswith('}'):
                            ...
                            current_obj_lines.insert(i, indent + '  required: ' + ('true' if insert_value else 'false') + ',')
                            break
